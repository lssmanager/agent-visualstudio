...
import { gatewayMethods } from '@lss/gateway-sdk';
 ...
 *
 * Fix B: import gatewayMethods from package root (@lss/gateway-sdk)
 * not from a deep src path which breaks tsc module resolution.
 */
@Injectable()
export class GatewayService {
  ...
  async getRuntimeCapabilityMatrix(): Promise<RuntimeCapabilityMatrix> {
    ...
    return {
      source: 'unknown',
      topology: { connect: false, disconnect: false, pause: false, reactivate: false, redirect: false, continue: false },
  ...
  async inspectChannels(): Promise<Array<{ channel: string; sessions: number; activeSessions: number }>> {
    ...
    return Array.from(map.entries()).map(([channel, stats]) => ({ channel, ...stats }));
  }
